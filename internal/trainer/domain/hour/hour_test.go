...
func TestNewAvailableHour_too_distant_date(t *testing.T) {
	...
	var hourErr hour.TooDistantDateError
	require.ErrorAs(t, err, &hourErr)
	assert.Equal(t, maxWeeksInFuture, hourErr.MaxWeeksInTheFutureToSet)
	assert.Equal(t, constructorTime, hourErr.ProvidedDate)
}
...
func TestNewAvailableHour_too_early_hour(t *testing.T) {
	...
	var hourErr hour.TooEarlyHourError
	require.ErrorAs(t, err, &hourErr)
	assert.Equal(t, factory.Config().MinUtcHour, hourErr.MinUtcHour)
	assert.Equal(t, tooEarlyHour, hourErr.ProvidedTime)
}
...
func TestNewAvailableHour_too_late_hour(t *testing.T) {
	...
	_, err := factory.NewAvailableHour(tooEarlyHour)
	var hourErr hour.TooLateHourError
	require.ErrorAs(t, err, &hourErr)
	assert.Equal(t, factory.Config().MaxUtcHour, hourErr.MaxUtcHour)
	assert.Equal(t, tooEarlyHour, hourErr.ProvidedTime)
}
...
func TestFactoryConfig_Validate(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			ExpectedErr: "MaxUtcHour should be value between 0 and 24, but is -1; MaxUtcHour (-1) can't be after MinUtcHour (10)",
		},
	}
