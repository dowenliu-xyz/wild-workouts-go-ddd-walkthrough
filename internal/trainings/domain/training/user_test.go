...
func TestIsUserAllowedToSeeTraining(t *testing.T) {
	attendee1, err := training.NewUser(uuid.New().String(), training.UserTypeAttendee())
	require.NoError(t, err)

	attendee2, err := training.NewUser(uuid.New().String(), training.UserTypeAttendee())
	require.NoError(t, err)

	trainer, err := training.NewUser(uuid.New().String(), training.UserTypeTrainer())
	require.NoError(t, err)
