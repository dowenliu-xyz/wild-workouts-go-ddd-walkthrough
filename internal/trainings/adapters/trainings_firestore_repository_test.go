...
func TestTrainingsFirestoreRepository_UpdateTraining(t *testing.T) {
	...
	err = repo.UpdateTraining(
		...
		training.MustNewUser(expectedTraining.UserUUID(), training.UserTypeAttendee()),
...
func TestTrainingsFirestoreRepository_get_and_update_another_users_training(t *testing.T) {
	...
	requestingUser := training.MustNewUser(uuid.New().String(), training.UserTypeAttendee())
...
func newRandomTrainingTime() time.Time {
	minSec := time.Now().AddDate(0, 0, 5).Unix()
	maxSec := time.Date(2070, 1, 0, 0, 0, 0, 0, time.UTC).Unix()
	delta := maxSec - minSec

	sec := rand.Int63n(delta) + minSec
...
func assertPersistedTrainingEquals(t *testing.T, repo adapters.TrainingsFirestoreRepository, tr *training.Training) {
	persistedTraining, err := repo.GetTraining(
		context.Background(),
		tr.UUID(),
		training.MustNewUser(tr.UserUUID(), training.UserTypeAttendee()),
	)
	require.NoError(t, err)
